...
export const favorites = asyncHandler(async (req, res) => {
  ...
  try {
    const userId = req.user._id;      // added by verifyjwt
    ...
    if (isFavorite) {
      ...
      await user.save();
      return res.status(200).json(new ApiResponse(200, user.favorites, "Added to favorites"));
...
export const getfavorites = asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(userId).populate('favorites'); // optional: populate book details
    if (!user) throw new ApiError(404, "User not found");

    res.status(200).json(new ApiResponse(200, user.favorites, "User favorites fetched"));
});
...
export const removeFavorite = async (req, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      { $pull: { favorites: bookId } }, // removes bookId from favorites
      { new: true }
    );
